...
`utils/pyHDLC/jobs.yml` (arch anchors like `*SysDebian*`). Recommended per-version defaults:
...
- `ARG REGISTRY 'ghcr.io/hdl/debian/trixie'` is the trixie default. See
  `debian-trixie/klayout.dockerfile` and `debian-trixie/klayout/…` for the convention of
  version-specific recipes setting the registry default.
