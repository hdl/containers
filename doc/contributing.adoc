...
A helper script is provided in link:{repotree}utils/bin/dockerBuild[`utils/bin/dockerBuild`] to ease building images, by
hiding all common options.
...
A helper script is provided in link:{repotree}utils/bin/dockerDive[`utils/bin/dockerDive`], which supports one or two
arguments for specifying the image to be inspected.
The default registry prefix is `ghcr.io/hdl`, however, it can be overriden through envvar `HDL_REGISTRY`.

For instance, inspect image `ghcr.io/debian-buster/ghdl`:
