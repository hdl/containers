    ...
    def __call__(self, func):
        self._AppendAttribute(
            ...
            SwitchArgumentAttribute(
                "--dry-run",
                dest="dry_run",
                help="Print build commands but do not execute them.",
            ),
        )
        # ... add more if needed
        return func
...
class CLI(ArgParseMixin):
    ...
    def __init__(self):
        ...
        ArgParseMixin.__init__(
            ...
            description=textwrap.dedent(
                "Helper tool to build one or multiple images, to easily browse publicly available tags, and to generate graphs showing the dependencies between tags."
            ),
    ...
    @CommandAttribute(
        "hub", help="Show list of repos and tags at hub.docker.com/u/ghdl."
    )
    ...
    @WithBuildAttributes()
    @ArgumentAttribute(
        metavar="<Repo>", dest="Repo", type=str, help="Repo - hub repo name"
    )
    @ArgumentAttribute(
        metavar="<Tag>", dest="Tag", type=str, help="Tag - target image tag"
    )
    ...
    @WithBuildAttributes()
    @ArgumentAttribute(
        metavar="<Name>", dest="Name", type=str, help="Name - unique identifier"
    )
    @ArgumentAttribute(
        metavar="<Args>",
        dest="Args",
        type=str,
        nargs="*",
        help="Args - optional arguments/filters",
    )
    def HandleTask(self, args):
        task(args.Name, args.Args, args.dry_run)

    @CommandAttribute(
        "graph", help="Generate a dot graph from the multi-stage dockerfiles (WIP)."
    )
    def HandleGraph(self, _):
        dockerfiles = Path(__file__).parent / ".." / "dockerfiles"
        generate_graph(
            {
                dfile: parse_dockerfile(dockerfiles / dfile)
                for dfile in listdir(dockerfiles)
            }
        )
